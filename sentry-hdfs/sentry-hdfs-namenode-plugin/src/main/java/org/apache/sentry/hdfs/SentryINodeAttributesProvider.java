...
public class SentryINodeAttributesProvider extends INodeAttributeProvider
  ...
  class SentryPermissionEnforcer implements AccessControlEnforcer {
    ...
    public void checkPermission(String fsOwner, String supergroup,
      ...
      if ((pathElems.length > 1) && ("".equals(pathElems[0]))) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("Enforcing Permission : + " + Lists
              .newArrayList(fsOwner, supergroup, callerUgi.getShortUserName(),
                      ...
                      authzInfo.isManaged(pathElems), isBlessedUser(callerUgi)));
      }
      // Optimization : Perform permission checks only if requested path
      // is not (managed by sentry && caller is hive)
      if (!authzInfo.isManaged(pathElems) || !isBlessedUser(callerUgi)) {
    ...
    private boolean isBlessedUser(UserGroupInformation ugi) {
      return ugi.getShortUserName().equals(user) ||
              com.google.common.collect.Sets.newHashSet(ugi.getGroupNames())
                      .contains(group);
    }
  ...
  public class SentryINodeAttributes implements INodeAttributes {
    ...
    public String getUserName() {
      String user = SentryINodeAttributesProvider.this.user;
      if (!authzInfo.isStale()) {
        if (!authzInfo.doesBelongToAuthzObject(pathElements)) {
          user = defaultAttributes.getUserName();
        }
      }
      return user;
    }
    ...
    public String getGroupName() {
      String group = SentryINodeAttributesProvider.this.group;
      if (!authzInfo.isStale()) {
        if (!authzInfo.doesBelongToAuthzObject(pathElements)) {
          group = defaultAttributes.getGroupName();
        }
      }
      return group;
    }
    ...
    public FsPermission getFsPermission() {
      ...
      for (String[] prefixPath : authzInfo.getPathPrefixes()) {
        if (Arrays.equals(prefixPath, pathElements)) {
          returnPerm = FsPermission.createImmutable((short) (returnPerm
                  .toShort() | 0x01));
          break;
        }
      }
      if (!authzInfo.isStale()) {
        if (!authzInfo.doesBelongToAuthzObject(pathElements)) {
          returnPerm = defaultAttributes.getFsPermission();
        }
      }
      return returnPerm;
    }
    ...
    public AclFeature getAclFeature() {
      ...
      Map<String, AclEntry> aclMap = new HashMap<String, AclEntry>();
      ...
      if (!authzInfo.isStale()) {
        ...
        if (authzInfo.doesBelongToAuthzObject(pathElements)) {
          ...
          hasAuthzObj = false;
          f = defaultAttributes.getAclFeature();
        }
      } else {
        isStale = true;
        f = new SentryAclFeature(ImmutableList.copyOf(aclMap.values()));
  ...
  public INodeAttributes getAttributes(String[] pathElements,
    ...
    pathElements = "".equals(pathElements[0]) && pathElements.length > 1 ?
            ...
            pathElements;
    return authzInfo.isManaged(pathElements) ? new SentryINodeAttributes
